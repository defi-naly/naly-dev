...
export function DefiMachine({ progress }: { progress: number }) {
  ...
  useFrame((_, delta) => {
    ...
    const shrinkT = smoothstep((progress - 0.30) / 0.25);
    const emergeT = smoothstep((progress - 0.30) / 0.25);
    const edgeGrowT = smoothstep((progress - 0.55) / 0.15);
    const dissolveT = smoothstep((progress - 0.90) / 0.10);
