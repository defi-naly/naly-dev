...
const cards: CardData[] = [
  {
    offTitle: 'Bolted on AI chatbots',
    offResult: 'Faster execution of the wrong strategy.',
    onTitle: 'Mapped decision architecture first.',
    onResult: 'AI accelerates decisions that were already right.',
    stat: 'Structure > Speed',
  },
  {
    offTitle: 'Automated content creation',
    offResult: '10x the volume through a broken funnel.',
    onTitle: 'Fixed the funnel. Then scaled it.',
    onResult: 'Every piece compounds. Distribution is the asset.',
    stat: 'Assets > Output',
  },
  {
...
export default function MethodProblemToggle() {
  ...
  return (
    <div className="judgment-section">
      ...
      <div className="judgment-cards">
        {cards.map((card, i) => (
          <ProblemCard key={i} card={card} isOn={isOn} index={i} totalCards={cards.length} />
        ))}
      </div>
    </div>
  );
