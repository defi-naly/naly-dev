...
export function SovereigntyMachine({ progress }: { progress: number }) {
  ...
  // Dense edges fade in as ball shrinks to sun — makes it look more solid
  const denseEdgeIndices = useMemo(() => kNearestEdges(decentralized, 14), [decentralized]);

  // Icosahedron geometry for nodes (matches DeFi node shape)
  ...
  // Node data: seeded rotation speeds and drift seeds
  ...
  useFrame((_, delta) => {
    ...
    // Morph: 35-60% (25% window for dramatic transition)
    const morphT = smoothstep((progress - 0.35) / 0.25);

    // Sun transform: entire group shrinks + translates to top-left (0.80–1.0)
    const sunT = smoothstep((progress - 0.80) / 0.20);
    // End phase: sun continues shrinking and drifts up as it fades (progress 1.55–1.75)
    const endT = smoothstep((progress - 1.55) / 0.20);
    const sunScale = (1.0 - sunT * 0.55) * (1 - endT * 0.6); // shrink further at end
    const sunTargetX = -2.0;
    const sunTargetY = 1.5;
    groupRef.current.scale.setScalar(sunScale);
    groupRef.current.position.set(
      sunTargetX * sunT,
      0.2 + sunTargetY * sunT + endT * 0.8, // drift upward at end
      0,
    );

    // Current node positions (lerp centralized → decentralized + orbital drift)
    ...
    for (let i = 0; i < NODE_COUNT; i++) {
      ...
      // Orbital drift — 3x amplitude (0.06 vs 0.02)
    ...
    // Resilience demo: 75-80% — dim 1-2 nodes briefly (before node fade begins)
    ...
    // Update individual node groups (position, scale, rotation)
    for (let i = 0; i < NODE_COUNT; i++) {
      ...
      // Size transition: spokes grow during morph
      let scale = 0.8 * (1 - morphT) + 1.2 * morphT;

      // Hub birth effect: emerges from DeFi cloud at 0.06, contracts to normal by 0.14
      if (i === 0) {
        const birthT = smoothstep((progress - 0.06) / 0.08);
        scale *= 5 * (1 - birthT) + 1 * birthT; // 5x → 1x
      }
    ...
    // Hub flashes red AFTER "single point of failure" annotation appears (0.24+)
    ...
    // Star edge growth: edges extend outward from hub over progress 0.08 → 0.20
    const growT = smoothstep((progress - 0.18) / 0.12);

    // Update star edge positions — spoke endpoints lerp from hub → spoke based on growT
    ...
    for (let i = 0; i < starEdgeIndices.length; i += 2) {
      // Even index = hub endpoint (always at hub position)
      starPos.setXYZ(i, hubPos.x, hubPos.y, hubPos.z);
      // Odd index = spoke endpoint (lerp from hub → spoke)
    ...
    // Update mesh edge positions from current node positions
    ...
    // Update dense edge positions (same nodes, more connections)
  ...
  // Opacity calculations — nodes fade during sun transition, mesh topology persists
  const endFade = 1 - smoothstep((progress - 1.55) / 0.20);
  // Nodes fade out as mesh moves to sun position (0.80→0.95), leaving just edges
  const nodeSunFade = 1 - smoothstep((progress - 0.80) / 0.15);
  // Hub: emerges from DeFi cloud — fades in at 0.06 (when cloud is densest), full by 0.14
  ...
  // Dense edges fade in after mesh topology established (progress 0.60–0.75), fade at end
  const denseOpacity = smoothstep((progress - 0.60) / 0.15) * 0.08 * endFade;

  return (
    <group ref={groupRef} position={[0, 0.2, 0]}>
      {/* Nodes — individual wireframe icosahedra with per-node rotation */}
      {nodeData.map((_, i) => (
      ...
      {/* Dense edges — fade in as ball becomes sun */}
      {denseOpacity > 0.01 && (
