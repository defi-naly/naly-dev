...
interface Particle {
  dx: number; // px scatter X
  dy: number; // px scatter Y (negative = up)
  size: number; // px
  delay: number; // 0–1 within the char's dissolve window
}
...
function generateParticles(charIndex: number, count: number): Particle[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    particles.push({
      dx: (r1 - 0.5) * 80, // -40 to +40
      dy: -(20 + r2 * 60),  // -20 to -80 (upward)
      size: 1.5 + r3 * 1.5, // 1.5 to 3
      delay: r4 * 0.3,       // slight stagger within dissolve
    });
  }
  return particles;
}
...
/** Compute per-char dissolve 0–1 given stagger range */
...
export function Cover({ progress }: CoverProps) {
  ...
  // Once scroll has started, kill entrance CSS animations (releases fill-mode: both)
  const scrollStarted = progress > 0.01;

  // Hide fixed text layer once fully dissolved
